...
export function Bubble() {
  ...
  return (
    <div  className="flex flex-col items-center space-y-8">
      <div className="flex space-x-[-1px] ">  {/* increased negative margin for larger overlap */}
        {bubbles.map(bubble => (
          <div
            key={bubble.name}
            className={`${
              bubble.name === activeBubble ? 'mt-8' : ''
            } ${bubble.color} w-60 h-60 rounded-full flex items-center justify-center cursor-pointer bubble-mix transition-transform transform duration-300 hover:translate-y-2`}  // Changed hover effect direction
      ...
      <div className={`h-[22vh] w-[110vh] text-center p-16 ${bubbles.find(b => b.name === activeBubble).color} rounded-3xl`}>  {/* max width and 70% viewport height for text box */}
